...
router.get('/admin', authMiddleware, adminOnly, (req, res) => {
  try {
    ...
    // Get total users
    db.query('SELECT COUNT(*) as totalUsers FROM users WHERE role IN ("normal_user", "admin")', (err, userResults) => {
      ...
      db.query('SELECT COUNT(*) as totalStores FROM stores', (err, storeResults) => {
        ...
        db.query('SELECT COUNT(*) as totalRatings FROM ratings', (err, ratingResults) => {
          ...
          res.json({
            ...
            totalStores: storeResults[0].totalStores,
            totalRatings: ratingResults[0].totalRatings
          });
        });
      });
    });
...
router.get('/store-owner', authMiddleware, storeOwnerOnly, (req, res) => {
  try {
    ...
    db.query(
      `SELECT s.id, s.name, ROUND(AVG(r.rating), 2) as averageRating, COUNT(r.id) as totalRatings
       FROM stores s
